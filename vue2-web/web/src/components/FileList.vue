...
export default {
    ...
    computed: {
        ...mapState(['currentTab']),
        uploadUrl: () => {
            return `${process.env.NODE_ENV === 'production' ? `${location.origin}` : 'api'}/webssh/file/upload`
        },
        uploadData: function() {
            return {
    ...
    methods: {
        ...
        uploadProgress(e, f) {
            ...
            f.startWsProgree = false;
            if(!f.startWsProgree &&  e.percent>=10){
                //test ...
                f.startWsProgree = true;
            }
            if (e.percent === 50) {
                const ws = wshelper.newWs({url:`${(location.protocol === 'http:' ? 'ws' : 'wss')}://${location.host}${(process.env.NODE_ENV === 'production' ? '' : '/ws') + (store.getters.ctx ? store.getters.ctx : '')}/webssh/file/progress?token=${token}&id=${f.uid}`})
                ws.onmessage = e1 => {
        ...
        downloadFile() {
            ...
            const downloadUrl = `${prefix}/webssh/file/download?path=${this.downloadFilePath}&sshInfo=${this.$store.getters.sshReq}&token=${token}`
